...
- **全局快捷键** — 按 Win+V 随时随地呼出弹窗（Win+V 被系统占用时自动降级为 Win+Shift+V）
...
按 **Win+V**（或降级快捷键 **Win+Shift+V**）打开弹窗。

## 使用教程

### 入门

1. **安装** — 从 [Releases](https://github.com/codant0/aPaste/releases) 下载最新 `aPaste_0.1.1_x64-setup.exe` 并运行安装程序。
2. **启动** — 安装后 aPaste 自动启动，驻留在系统托盘中（查找剪贴板图标）。
3. **正常复制** — 继续使用 Ctrl+C 复制文本。aPaste 在后台静默记录所有内容。

### 呼出弹窗

按下全局快捷键（默认 **Win+V**）呼出弹窗。如果 Win+V 被 Windows 系统占用，则自动降级使用 **Win+Shift+V**。窗口出现在主显示器右下角，始终置顶于其他窗口之上。

| 操作 | 方式 |
|---|---|
| 显示弹窗 | `Win+V`（被占用时降级为 `Win+Shift+V`） |
| 隐藏弹窗 | `Esc` 或点击窗口外部 |
| 从托盘显示 | 左键托盘图标，或右键 →「显示窗口」 |

### 浏览和搜索

弹窗按时间倒序显示最近的剪贴板记录，每条展示：

- **时间戳** — 相对时间（刚刚、5 分钟前、昨天 等）
- **来源应用** — 文本被复制时所在的应用（如 记事本、Chrome）
- **内容预览** — 复制文本的前 120 个字符

**搜索：** 在搜索栏输入关键词进行过滤。aPaste 使用 FTS5 全文搜索和前缀匹配 — 输入 `npm` 可找到 "npm install tauri"，输入 `React` 找到所有 React 相关条目。

### 粘贴条目

1. 按 `Win+V`（或 `Win+Shift+V`）打开弹窗。
2. 使用 `↑` / `↓` 方向键选中目标条目。
3. 按 `Enter` — 弹窗自动隐藏，选中文本粘贴到当前焦点应用。
...
| 操作 | 方式 |
|---|---|
| 删除单条 | 悬停条目，点击 ✕ 按钮；或选中后按 `Delete` 键 |
| 清空非收藏项 | 点击标题栏「清空全部」按钮（需二次确认，防止误操作） |

> **收藏项受保护：**「清空全部」会跳过已收藏条目，不会被删除。

### 使用收藏

收藏功能让你固定重要的剪贴板条目，使其免受清理影响。
...
**查看收藏：**
点击搜索栏下方的「收藏项」标签，仅显示已收藏条目。

**取消收藏：**
悬停在已收藏条目上，再次点击 ★ 即可取消。在收藏标签中，条目会立即从视图中消失。

**在收藏中搜索：**
切换到「收藏项」标签，在搜索栏输入关键词 — 仅在收藏项中搜索匹配。
...
|---|---|
| **外观** | 深色 / 浅色主题 |
| **历史记录** | 最大保留条数（100–10000，默认 1000）、最大保留天数（1–365，默认 30） |
| **快捷键** | 点击录制新的全局快捷键组合 |
| **系统** | 开机自动启动（开关切换） |
...
| 快捷键 | 功能 |
|---|---|
| `Win+V` / `Win+Shift+V` | 呼出弹窗（可在设置中自定义） |
| `↑` / `↓` | 导航条目 |
| `Enter` | 粘贴选中条目 |
| `Delete` | 删除选中条目 |
| `Esc` | 隐藏弹窗 / 取消 |
